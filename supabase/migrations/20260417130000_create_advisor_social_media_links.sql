create table if not exists advisor_social_media_links (
...
alter table advisor_social_media_links enable row level security;
...
begin
  if not exists (
    select 1
    from pg_policies
    where tablename = 'advisor_social_media_links'
      and policyname = 'Allow all'
  ) then
    execute 'create policy "Allow all" on advisor_social_media_links for all using (true) with check (true)';
  end if;
end
$$;
