...
import {
  ...
  createEmptyAdvisorSocialMediaFormState,
  mapAdvisorSocialMediaRow,
  type AdvisorSocialMediaFormState,
  type AdvisorSocialMediaItem,
  type AdvisorSocialMediaItemRow,
} from '@/lib/advisor-social-media-items'
...
export default function AdvisorSocialMediaManager() {
  const [items, setItems] = useState<AdvisorSocialMediaItem[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [formState, setFormState] = useState<AdvisorSocialMediaFormState>(
    createEmptyAdvisorSocialMediaFormState
  )
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editingState, setEditingState] = useState<AdvisorSocialMediaFormState>(
    createEmptyAdvisorSocialMediaFormState
  )
  ...
  function startEdit(item: AdvisorSocialMediaItem) {
  ...
  function cancelEdit() {
    setEditingId(null)
    setEditingState(createEmptyAdvisorSocialMediaFormState())
  }
  ...
  async function handleUpdate(itemId: string) {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((item) =>
          item.id === itemId ? mapAdvisorSocialMediaRow(data as AdvisorSocialMediaItemRow) : item
        )
      )
      cancelEdit()
    } catch (updateError) {
  ...
  return (
    <section className="space-y-6" aria-labelledby="advisor-social-media-manager-heading">
      <div className="space-y-2">
        <h2
          id="advisor-social-media-manager-heading"
          className="text-xl font-semibold text-gray-900"
        >
          Danışmanlar/SM Linkleri
        </h2>
        <p className="max-w-3xl text-sm text-gray-500">
          Danışman ve sosyal medya odaklı kaynakları ayrı tabloda yönetin.
        </p>
      </div>

      <AccordionCard
        defaultOpenId="new-advisor-social"
        items={[
          {
            id: 'new-advisor-social',
            title: 'Yeni Link Ekle',
            accentColor: '#0F9D58',
            children: (
              <form
                onSubmit={handleCreate}
                className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[0.8fr_1.2fr_1fr_0.6fr]"
              >
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormState((state) => ({
                        ...state,
                        platform: e.target.value as AdvisorSocialMediaFormState['platform'],
                      }))
                    }
                    className={INPUT_CLS}
                  >
                    {ADVISOR_SOCIAL_PLATFORMS.map((platform) => (
                ...
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormState((state) => ({ ...state, link: e.target.value }))
                    }
                    placeholder="https://..."
                    className={INPUT_CLS}
                    required
                ...
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormState((state) => ({
                        ...state,
                        addedBy: e.target.value as AdvisorSocialMediaFormState['addedBy'],
                      }))
                    }
                    className={INPUT_CLS}
                  >
                    {ADVISOR_SOCIAL_AUTHORS.map((author) => (
            ...
            <div className="hidden overflow-x-auto rounded-2xl border border-[rgba(66,133,244,0.1)] bg-white shadow-[0_10px_20px_rgba(60,64,67,0.04)] md:block">
              <table className="min-w-full divide-y divide-gray-50 text-sm">
                <thead className="bg-gray-50/80">
                  <tr>
                    {['Platform', 'Açıklama', 'Link', 'Kim', 'İşlemler'].map((column) => (
                      <th
                        key={column}
                        scope="col"
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setEditingState((state) => ({
                                  ...state,
                                  platform:
                                    e.target.value as AdvisorSocialMediaFormState['platform'],
                                }))
                              }
                              className={INPUT_CLS}
                            >
                        ...
                        <td className="w-[28%] px-4 py-3.5">
                          {rowIsEditing ? (
                            <input
                              ...
                              onChange={(e) =>
                                setEditingState((state) => ({ ...state, link: e.target.value }))
                              }
                              className={INPUT_CLS}
                            />
                          ) : item.link ? (
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setEditingState((state) => ({
                                  ...state,
                                  addedBy:
                                    e.target.value as AdvisorSocialMediaFormState['addedBy'],
                                }))
                              }
                              className={INPUT_CLS}
                            >
